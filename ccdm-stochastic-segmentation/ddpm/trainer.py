...
class Trainer:
    ...
    def train_step(self, engine: Engine, batch) -> dict:
        ...
        image, first_stage_x0, second_stage_x0 = batch
        self.model.train()
        if self.params['feature_cond_encoder']['train']:
            self.feature_cond_encoder.train()

        self.shape = first_stage_x0.shape[1:]

        device = idist.device()
        image = image.to(device, non_blocking=True)
        first_stage_x0 = first_stage_x0.to(device, non_blocking=True)
        second_stage_x0 = second_stage_x0.to(device, non_blocking=True)
        ...
        batch_size = first_stage_x0.shape[0]

        # Sample a random step and generate gaussian noise
        if self.params['optim']['skip_type'] == "optimized":
            optimizer = StepOptim(self.params['optim']['noise_schedule'])
            t, _ = optimizer.get_ts_lambdas(self.time_steps + 1, 1, self.params['optim']['optimized_type'])
            t = t.to(device).to(torch.float32)
        else:
            t = torch.randint(1, self.time_steps + 1, size=(batch_size,), device=device)
        # t = torch.randint(1, self.time_steps + 1, size=(batch_size,), device=device)
        xt = self.diffusion_model.q_xt_given_x0(first_stage_x0, t).sample()
        ...
        prob_xtm1_given_xt_x0 = self.diffusion_model.theta_post(xt, first_stage_x0, t)
        ...
        mask = self.class_weights[first_stage_x0.argmax(dim=1)]
...
def build_engine(trainer: Trainer,
    ...
    if idist.get_local_rank() == 0:
        ...
        if params["wandb"]:
            tb_logger = WandBLogger(project=params["wandb_project"], entity='bulebed1754', config=params)
