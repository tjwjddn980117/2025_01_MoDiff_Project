...
def mopology_gradient(input_condition, kernel_size, padding):
        dilation = F.max_pool2d(input_condition, kernel_size=kernel_size, stride=1, padding=padding)
        erosion = -F.max_pool2d(-input_condition, kernel_size=kernel_size, stride=1, padding=padding)

        # 모폴로지 그라디언트: 팽창 - 침식
        first_gradient = dilation - erosion
        return first_gradient


def make_gradient(input_condition, step):
    for i in range(1,step):
        grad = mopology_gradient(input_condition,2*i+1,i)
        if i==1:
            result_grad = grad
        else:
            result_grad = th.add(result_grad,grad)
    return result_grad


def reshape_for_attention(x):
    batch, channel, height, width = x.shape
    return x.view(batch, channel, height * width).permute(0, 2, 1)  # (batch, height*width, channel)


class CrossAttention(nn.Module):
    def __init__(self, d_model, num_heads=1, ff_hidden_dim=256):
        super(CrossAttention, self).__init__()
        self.num_heads = num_heads
        self.d_model = d_model
        self.head_dim = d_model // num_heads

        assert self.head_dim * num_heads == d_model, "Embedding dimension must be divisible by number of heads"
        ...
        self.feed_forward = nn.Sequential(
            nn.Linear(d_model, ff_hidden_dim, bias=False),
            nn.ReLU(),
            nn.Linear(ff_hidden_dim, d_model, bias=False)
        )
    ...
    def forward(self, query, key, value):
        ...
        attn_scores = th.matmul(query, key.transpose(-2, -1)) * self.scale  # (..., query_len, key_len)
        attn_probs = self.softmax(attn_scores)
        attn_output = th.matmul(attn_probs, value)  # (..., query_len, head_dim)
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, input_condition, feature_condition, timesteps, y=None):
        ...
        grad = make_gradient(input_condition,5)
        
        grad_reshaped = reshape_for_attention(grad)  # Query
        input_condition_reshaped = reshape_for_attention(input_condition)  # Key, Value
        d_model = grad_reshaped.size(-1)
        
        cross_attention = CrossAttention(d_model=d_model).to(grad.device)
        latent_representation = cross_attention(grad_reshaped, input_condition_reshaped, input_condition_reshaped)  # (batch, query_len, channel)

        # 잠재 변수를 원래 차원으로 복원 (batch, channel, height, width)
        latent_representation = latent_representation.view(input_condition.shape[0], -1, 1).permute(0,2,1).contiguous()
        latent_representation = latent_representation.view(input_condition.shape[0],-1,input_condition.shape[2],input_condition.shape[3])
        ...
        x = th.cat([x, latent_representation], dim=1)
