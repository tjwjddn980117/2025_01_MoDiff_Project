...
from utils.save_results import save_results, save_results_while_batch
from utils import get_conf_map_accumulate,m_scheduler,get_model_size, save_pixel_images, get_conf_map_accumulate_with_16_sampling, get_16_sampling
...
class Engine:
    ...
    def train(self, data):
        ...
        for self.epoch in tqdm(range(start_epoch, self.exp_config.epochs)):
            ...
            for ii,(patch_arrangement, masks_arrangement, prob_gt, path) in tqdm(enumerate(data.train)):
    ...
    def validate(self, data, vis_batch_size = 3):
        ...
        with torch.no_grad():
            metrics_list = []
            for ii,(patch_arrangement, masks_arrangement, prob_gt, path) in enumerate(data.validation):
                patch_arrangement = patch_arrangement.cuda()
                masks_arrangement = masks_arrangement.cuda()
                prob_gt = prob_gt.cuda()
    ...
    def test(self, dataloader, log_dir):
        ...
        with torch.no_grad():
            ...
            all_pixel_image = None
            path_list = []
            for ii, (patch_arrangement, masks_arrangement, prob_gt, path) in enumerate(tqdm(dataloader)):
                ...
                metrics,prediction,prob = self.net.forward(patch_arrangement, masks_arrangement, prob_gt, val=True)
                if ii == 0:
                    # Convert lists to tensors for shape printing
                    metrics_tensor = torch.stack(metrics) if isinstance(metrics, list) else metrics
                    prediction_tensor = torch.stack(prediction) if isinstance(prediction, list) else prediction
                    prob_tensor = torch.stack(prob) if isinstance(prob, list) else prob

                    # Print the shapes of the tensors
                    print()
                    print(f'metrics_shape: {metrics_tensor.shape}')
                    print(f'metrics: {metrics_tensor}')
                    print(f'prediction_shape: {prediction_tensor.shape}')
                    print(f'prob_shape: {prob_tensor.shape}')
                    print(f'patch_arrangement_shape: {patch_arrangement.shape}')
                    print(f'masks_arrangement_shape: {masks_arrangement.shape}')
                    print(f'prob_gt_shape: {prob_gt.shape}')
                    #print(path)
                    prediction_tensor = prediction_tensor.cpu()
                    unique_values = torch.unique(prediction_tensor)
                    if torch.equal(unique_values, torch.tensor([0, 1])):
                        print("binary prediction")
                    else:
                        print("nonbinary predictino")

                metrics_list.append([metric_.nanmean().item() for metric_ in metrics])  # get mean if parallel is used
                # all_pixel_image = get_conf_map_accumulate_with_16_sampling(prediction, all_pixel_image)
                sample_image = get_16_sampling(prediction)
                save_results_while_batch(sample_image, path,'C:/Users/Seo/Downloads/MoSE-AUSeg-main/MoSE_LIDC_results_32_sample')
                # all_pixel_conf, all_gt_conf, all_pixel_image = get_conf_map_accumulate(prediction, prob, masks_arrangement, prob_gt,
                #                                                self.exp_config.n_classes, all_pixel_conf, all_gt_conf, all_pixel_image)
                
                #if ii < 5:
                #    # Print the shapes of the tensors
                #    print()
                #    print(ii)
                #    print(f'all_pixel_conf_shape: {all_pixel_conf.shape}')
                #    print(f'all_gt_conf_shape: {all_gt_conf.shape}')
                #    print(f'all_pixel_image_shape: {all_pixel_image.shape}')

                del patch_arrangement,masks_arrangement,prob_gt, metrics, prediction, sample_image

            print('finish!')
            # print(f'all_pixel_image_shape: {all_pixel_image.shape}')
            # save_results(all_pixel_image, path_list, 'C:/Users/Seo/Downloads/MoSE-AUSeg-main/Results')
            # calculate ECE
            # save_pixel_images(pixel_image=all_pixel_image, output_dir='C:/Users/Seo/Downloads/MoSE-AUSeg-main/outputs',threshold=0.75)
